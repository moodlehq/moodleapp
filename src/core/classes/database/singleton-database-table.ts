 ...
 */
export abstract class CoreSingletonDatabaseTable<
...
> extends CoreDatabaseTable<DBRecord, PrimaryKeyColumn, PrimaryKey> {
    ...
    async initialize(): Promise<void> {
        ...
        const activeTables = CoreSingletonDatabaseTable.ACTIVE_TABLES.get(this.database) ?? new Set();

        if (activeTables.has(this.tableName)) {
            const message = `Trying to create multiple instances of a singleton table for '${this.tableName}'`;

            if (!CoreConstants.BUILD.isProduction) {
                throw new Error(message);
            }

            CoreSingletonDatabaseTable.LOGGER.warn(message);
        }

        activeTables.add(this.tableName);
        CoreSingletonDatabaseTable.ACTIVE_TABLES.set(this.database, activeTables);
    }
    ...
    async destroy(): Promise<void> {
        ...
        const activeTables = CoreSingletonDatabaseTable.ACTIVE_TABLES.get(this.database);

        activeTables?.delete(this.tableName);

        if (activeTables?.size === 0) {
            CoreSingletonDatabaseTable.ACTIVE_TABLES.delete(this.database);
        }
    }
