...
export class CoreTime {
    ...
    static formatTime(seconds: number, precision = 2): string {
        ...
        let remaining = totalSecs - (years * CoreTimeConstants.SECONDS_YEAR);

        const days = Math.floor(remaining / CoreTimeConstants.SECONDS_DAY);
        remaining = remaining - (days * CoreTimeConstants.SECONDS_DAY);

        const hours = Math.floor(remaining / CoreTimeConstants.SECONDS_HOUR);
        remaining = remaining - (hours * CoreTimeConstants.SECONDS_HOUR);

        const mins = Math.floor(remaining / CoreTimeConstants.SECONDS_MINUTE);
        const secs = remaining - (mins * CoreTimeConstants.SECONDS_MINUTE);
