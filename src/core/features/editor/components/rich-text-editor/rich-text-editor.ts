...
const MIN_HEIGHT = 200; // Minimum height of the editor.
const DRAFT_AUTOSAVE_FREQUENCY = 30000;
...
export class CoreEditorRichTextEditorComponent implements AfterViewInit, OnDestroy, OnInit {
    ...
    protected onChange(content: string): void {
        ...
        NgZone.run(() => {
            if (CoreDom.htmlIsBlank(content)) {
                ...
                this.restoreExternalContent(container);
    ...
    protected async restoreDraft(): Promise<void> {
        ...
        if (draftText !== '' && this.control && draftText != this.control.value) {
            ...
            if (entry.drafttext != entry.originalcontent) {
                // Notify the user.
                this.dynamicComponent?.callComponentMethod('onDraftRestore');
            }
        }
    ...
    protected autoSaveDrafts(): void {
        this.autoSaveInterval = window.setInterval(async () => {
            ...
            try {
                ...
                this.dynamicComponent?.callComponentMethod('onDraftSave');
            } catch {
                // Error saving draft.
            }
    ...
    protected setContent(content: string): void {
        ...
        this.treatExternalContent(container);
     ...
     */
    protected treatExternalContent(container: HTMLElement): void {
     ...
     */
    protected restoreExternalContent(container: HTMLElement): void {
        if (!CoreSites.isLoggedIn()) {
            // Only treat external content if the user is logged in.
            return;
        }
