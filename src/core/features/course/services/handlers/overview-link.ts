    ...
    getActions(
        ...
        return [{
            action: async (siteId): Promise<void> => {
                if ((!siteId || siteId === CoreSites.getCurrentSiteId()) && CoreCourse.currentViewIsCourse(courseId)) {
                    CoreCourse.selectCourseTab('CoreCourseOverview');
                } else {
                    await CoreCourseHelper.getAndOpenCourse(courseId, { selectedTab: 'CoreCourseOverview' }, siteId);
                }
            },
        }];
    }
