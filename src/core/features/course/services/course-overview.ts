...
export class CoreCourseOverviewService {
     ...
     *
     * @param courseId Course ID.
     * @param modName Module name to expand. E.g. 'glossary'.
     * @param title Title to pass to list-mod-type page if overview not available.
     * @param siteId Site ID. If not defined, current site.
     */
    async navigateToCourseOverview(courseId: number, modName?: string, title?: string, siteId?: string): Promise<void> {
        ...
        if (CoreCourseOverview.canGetInformation()) {
            ...
            await CoreCourseHelper.getAndOpenCourse(
                courseId,
                { selectedTab: CORE_COURSE_OVERVIEW_OPTION_NAME, expand: modName },
                siteId,
            );
