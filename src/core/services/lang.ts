...
export class CoreLangProvider {
    ...
    getParentLanguage(): string | undefined {
        const parentLang = Translate.instant(MoodleTranslateLoader.PARENT_LANG_KEY);
        if (parentLang !== '' && parentLang !== MoodleTranslateLoader.PARENT_LANG_KEY && parentLang !== this.currentLanguage) {
            return parentLang;
        }
    }
    ...
    clearCustomStrings(): void {
        this.getTranslateLoader().clearCustomStrings(this.currentLanguage);
    ...
    async loadCustomStrings(strings: string): Promise<void> {
        ...
        if (!strings) {
            this.getTranslateLoader().clearCustomStrings();
            this.customStringsRaw = '';
