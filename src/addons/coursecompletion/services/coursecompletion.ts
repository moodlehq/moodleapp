...
export class AddonCourseCompletionProvider {
     ...
     * @deprecated since 5.1. Use CoreCourseCompletionHelper.isCompletionEnabledInSite instead.
     */
    isCompletionEnabledInSite(site?: CoreSite): boolean {
        return CoreCourseCompletionHelper.isCompletionEnabledInSite(site);
    }
     ...
     * @param site Site. If not defined, use current site.
     * @returns True if available.
     * @deprecated since 5.1. Use CoreCourseCompletionHelper.isCompletionEnabledInCourse instead.
     */
    isCompletionEnabledInCourse(course: CoreCourseAnyCourseData, site?: CoreSite): boolean {
        return CoreCourseCompletionHelper.isCompletionEnabledInCourse(course, site);
    }
    ...
    isPluginViewEnabled(): boolean {
        return CoreSites.isLoggedIn() && CoreCourseCompletionHelper.isCompletionEnabledInSite();
    }
    ...
    async isPluginViewEnabledForCourse(courseId?: number, preferCache = true): Promise<boolean> {
        if (!courseId || !CoreCourseCompletionHelper.isCompletionEnabledInSite()) {
            return false;
        }
        ...
        return CoreCourseCompletionHelper.isCompletionEnabledInCourseObject(course) && course.completionhascriteria !== false;
    }
