 ...
 * @ngdoc service
 * @name $mmaModQuizQuestionRandomSaMatchHandler
 */
.factory('$mmaModQuizQuestionRandomSaMatchHandler', function() {
    ...
    self.getDirectiveName = function(question) {
        return 'mma-mod-quiz-question-random-sa-match';
    };
...
.run(function($mmAddonManager) {
    ...
    if ($mmaModQuizQuestionsDelegate) {
        $mmaModQuizQuestionsDelegate.registerHandler('mmaModQuizRandomSaMatch', 'qtype_randomsamatch',
                                                        '$mmaModQuizQuestionRandomSaMatchHandler');
    }
});
