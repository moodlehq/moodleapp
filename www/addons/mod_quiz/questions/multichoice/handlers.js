 ...
 * @ngdoc service
 * @name $mmaModQuizQuestionMultichoiceHandlers
 */
.factory('$mmaModQuizQuestionMultichoiceHandler', function() {
    ...
    self.getDirectiveName = function(question) {
        return 'mma-mod-quiz-question-multichoice';
    };
...
.run(function($mmAddonManager) {
    ...
    if ($mmaModQuizQuestionsDelegate) {
        $mmaModQuizQuestionsDelegate.registerHandler('mmaModQuizMultichoice', 'qtype_multichoice',
                                                        '$mmaModQuizQuestionMultichoiceHandler');
    }
});
