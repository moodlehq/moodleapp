 ...
 * @ngdoc service
 * @name $mmaModQuizQuestionCalculatedMultiHandler
 */
.factory('$mmaModQuizQuestionCalculatedMultiHandler', function() {
    ...
    self.getDirectiveName = function(question) {
        return 'mma-mod-quiz-question-calculated-multi';
    };
...
.run(function($mmAddonManager) {
    ...
    if ($mmaModQuizQuestionsDelegate) {
        $mmaModQuizQuestionsDelegate.registerHandler('mmaModQuizCalculatedMulti', 'qtype_calculatedmulti',
                                                        '$mmaModQuizQuestionCalculatedMultiHandler');
    }
});
