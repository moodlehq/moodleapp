...
/**
 * Gap select question handlers.
 *
 * @module mm.addons.mod_quiz
 * @ngdoc service
 * @name $mmaModQuizQuestionGapSelectHandler
 */
.factory('$mmaModQuizQuestionGapSelectHandler', function() {
    ...
    self.getDirectiveName = function(question) {
        return 'mma-mod-quiz-question-gap-select';
    };
...
.run(function($mmAddonManager) {
    ...
    if ($mmaModQuizQuestionsDelegate) {
        $mmaModQuizQuestionsDelegate.registerHandler('mmaModQuizGapSelect', 'qtype_gapselect',
                                                        '$mmaModQuizQuestionGapSelectHandler');
    }
});
