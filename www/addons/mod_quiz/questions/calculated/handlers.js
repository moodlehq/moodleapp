 ...
 * @ngdoc service
 * @name $mmaModQuizQuestionCalculatedHandler
 */
.factory('$mmaModQuizQuestionCalculatedHandler', function() {
    ...
    self.getDirectiveName = function(question) {
        return 'mma-mod-quiz-question-calculated';
    };
...
.run(function($mmAddonManager) {
    ...
    if ($mmaModQuizQuestionsDelegate) {
        $mmaModQuizQuestionsDelegate.registerHandler('mmaModQuizCalculated', 'qtype_calculated',
                                                        '$mmaModQuizQuestionCalculatedHandler');
    }
});
